...
export default function StudyBuddyApp() {
  ...
  const [selectedListing, setSelectedListing] = useState<StudyListing | null>(null);
  ...
  return (
    <div style={{
      minHeight: '100vh',
      background: 'linear-gradient(to bottom, #fef6e4 0%, #f3d2c1 100%)',
      ...
      {currentPage === 'home' && <HomePage user={user} profile={profile} onNavigate={navigate} />}
      {currentPage === 'create-listing' && <CreateListingPage user={user} onNavigate={navigate} />}
      {currentPage === 'listing-detail' && <ListingDetailPage listing={selectedListing} user={user} onNavigate={navigate} />}
      {currentPage === 'session' && <SessionPage session={selectedSession} user={user} onNavigate={navigate} />}
    </div>
  );
}
...
function HomePage({ user, profile, onNavigate }: { user: User | null; profile: Profile | null; onNavigate: (page: Page, data?: any) => void }) {
    ...
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px' }}>
        ...
        <div style={{ marginBottom: '40px' }} className="fade-in">
          ...
          <div style={{
            ...
            {mySessions.map((session) => (
              ...
              >
                <h3 style={{ color: '#001858', marginBottom: '8px', fontSize: '1.1rem' }}>
      ...
      {listings.length === 0 && (
        <div style={{
          ...
          <p style={{ fontSize: '18px' }}>Пока нет объявлений. Создайте первое!</p>
        </div>
      )}
    </div>
  );
...
function CreateListingPage({ user, onNavigate }: { user: User | null; onNavigate: (page: Page) => void }) {
    ...
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '40px 20px' }}>
      ...
      <div className="card fade-in">
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div>
            <label style={{ display: 'block', marginBottom: '8px', fontWeight: 600, color: '#001858' }}>
              Название
            </label>
            <input
              type="text"
...
function ListingDetailPage({ listing, user, onNavigate }: { listing: StudyListing | null; user: User | null; onNavigate: (page: Page, data?: any) => void }) {
...
function SessionPage({ session, user, onNavigate }: { session: StudySession | null; user: User | null; onNavigate: (page: Page) => void }) {
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '20px', height: '100vh', display: 'flex', flexDirection: 'column' }}>
      <div style={{
        display: 'flex',
        justifyContent: 'space-between',
        alignItems: 'center',
