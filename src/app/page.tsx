...
import {
  Flame, BookOpen, SlidersHorizontal,
...
export default function HomePage() {
  ...
  useEffect(() => { checkUser(); }, []);
  useEffect(() => { if (user) { loadListings(); loadActiveSessions(); } }, [user, searchQuery]);
  ...
  // Load ALL active/pending sessions for the user — regardless of how they were created
  ...
  const availableCities = useMemo(() =>
    Array.from(new Set(listings.map(l => l.city).filter((c): c is string => !!c))).sort(),
    [listings]);
    ...
    <div className="space-y-4">
      ...
      <FilterSection title="Формат">
        <div className="grid grid-cols-3 gap-1.5">
          {([
            ['all', '🌐', 'Любой'], ['online', '💻', 'Онлайн'], ['offline', '🤝', 'Офлайн'],
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: 'var(--app-bg)' }}>
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {/* ── Active sessions (ALL — from listing or from messages) ── */}
