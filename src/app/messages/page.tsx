...
function ConvMenu({
  ...
  return (
    ...
    >
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 w-44 rounded-xl border border-border shadow-xl z-20 overflow-hidden py-1 bg-card">
          {!isBlocked ? (
            <button
              onClick={() => {
                setOpen(false);
...
export default function MessagesPage() {
  ...
  const handleClose = async (conv: DirectConversation) => {
    if (!user) return;
    if (
      !confirm("Закрыть этот диалог? Он исчезнет из вашего списка сообщений.")
    )
      return;
    await supabase
      .from("direct_conversations")
  ...
  const handleBlock = async (conv: DirectConversation) => {
    ...
    if (
      !confirm(
        "Заблокировать этого пользователя? Он не сможет отправлять сообщения в этом чате.",
      )
    )
      return;
    const { data } = await supabase
    ...
    if (data) {
      setConversations((prev) =>
        prev.map((c) => (c.id === conv.id ? { ...c, blocked_by: user.id } : c)),
      );
    }
  };
                      ...
                      <div className="flex-1 min-w-0">
                        {conv.listing && (
                          <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                            ...
                            {isBlocked && (
                              <span className="shrink-0 px-1.5 py-0.5 rounded-full text-[10px] font-medium border text-destructive bg-destructive/10 border-destructive/20">
                                заблокирован
                              </span>
                            )}
                          </div>
