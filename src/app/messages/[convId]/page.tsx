...
import { ArrowLeft, Send, Loader2, BookOpen, Users, Ban } from "lucide-react";
...
export default function DirectChatPage() {
  ...
  const init = async () => {
    ...
    const otherId =
      convData.user1_id === u.id ? convData.user2_id : convData.user1_id;
  ...
  const isCurrentUserBlocked =
    !!conv?.blocked_by && conv.blocked_by !== user?.id;
  ...
  const sessionBadge = () => {
    ...
    if (session.status === "active")
      return {
        text: "сессия активна",
        cls: "text-secondary bg-secondary/10 border-secondary/20",
      };
      ...
      items.push(
        ...
        >
          ...
          <div className={cn("max-w-[70%]", !isFirst && !isOwn && "ml-10")}>
            {!isOwn && isFirst && (
              <p className="text-xs font-semibold text-primary mb-1 ml-1">
                {otherUser?.full_name}
              </p>
            )}
            <div
            ...
            <p
              className={cn(
                ...
                isOwn ? "text-right" : "text-left ml-1",
              )}
            >
          ...
          <div className="flex items-center gap-3 flex-1 min-w-0">
            {otherUser?.avatar_url ? (
              <img
                ...
                alt="avatar"
                className={cn(
                  "w-10 h-10 sm:w-12 sm:h-12 rounded-xl object-cover shrink-0",
              ...
              <div
                className={cn(
                  "w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center text-primary-foreground font-bold shrink-0",
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-2 flex-wrap mt-0.5">
                ...
                {currentUserIsBlocker && (
                  <span className="text-[10px] px-2 py-0.5 rounded-full font-medium text-destructive bg-destructive/10 border border-destructive/20 shrink-0">
                    заблокирован
                  </span>
                )}
              </div>
            ...
            <div className="p-4 border-t border-border shrink-0">
              {currentUserIsBlocker && (
                <p className="text-xs text-center text-destructive/70 mb-3">
                  Вы заблокировали этого пользователя — он не может вам
                  написать, но вы можете
                </p>
              )}
