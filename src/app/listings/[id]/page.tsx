...
export default function ListingDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8 max-w-3xl mb-20">
        <Button
          variant="ghost"
          onClick={() => router.push("/")}
          className="mb-6"
        ...
        <div className="rounded-2xl p-8 border border-border bg-card">
          <div className="flex justify-between items-start mb-6 gap-4">
            ...
            <span className="px-3 py-1.5 rounded-lg text-sm font-medium shrink-0 bg-muted text-muted-foreground">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-6">
            ...
            <Info label="Формат" icon={<UserIcon className="h-4 w-4" />}>
              <span className="text-foreground">
                {formatLabels[listing.format ?? "any"]}
              </span>
            </Info>
            {listing.city && (
            ...
            <>
              {proposed ? (
                <div className="rounded-xl p-5 text-center border border-secondary/20 bg-secondary/5">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Ждите подтверждения от{" "}
                    {listing.profiles?.full_name?.split(" ")[0] ?? "автора"}
                  </p>
                  <button
              ...
              {!proposed && (
                <p className="text-xs text-muted-foreground/60 text-center mt-3">
                  «Предложить сессию» отправит запрос — сессия начнётся только
                  после подтверждения
                </p>
              )}
            </>
